...
class BrowsershotLambda extends Browsershot
{
     ...
     */
    public function saveToS3(string $targetPath)
    {
        $this->setOption('s3', [
            'path' => $targetPath,
            'region' => config('sidecar.s3_region'),
            'bucket' => config("sidecar.s3_bucket")
        ]);
        ...
        if (empty($output)) {
            throw CouldNotTakeBrowsershot::chromeOutputEmpty("$targetPath on S3 disk: $s3_bucket", $output, $command);
        }
    }
}
